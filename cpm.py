...
class CPM:
    ...
    def cnn_fc(self, input_, is_train, name):
      ...
      with tf.variable_scope(name) as scope:
        conv1 = self.conv_layer(input_, 5, 96,
            'conv1', is_BN, trainable)
        pool1 = tf.nn.max_pool(conv1, ksize=[1, 3, 3, 1],
                strides=[1, 2, 2, 1],
                padding="SAME", name="pool1")

        conv2 = self.conv_layer(pool1, 5, 256,
            'conv2', is_BN, trainable)
        pool2 = tf.nn.max_pool(conv2, ksize=[1, 3, 3, 1],
                strides=[1, 2, 2, 1],
                padding="SAME", name="pool2")

        conv3 = self.conv_layer(pool2, 5, 384,
            'conv3', is_BN, trainable)
        ...
        conv5 = self.conv_layer(conv4, 3, 256,
            'conv5', is_BN, trainable)
        conv6 = self.conv_layer(conv5, 3, 256,
            'conv6', is_BN, trainable)
        conv7 = self.conv_layer(conv6, 3, 256,
            'conv7', is_BN, trainable)
        conv8 = self.conv_layer(conv7, 3, 128,
            'conv8', is_BN, trainable)
        conv9 = self.conv_layer(conv8, 1, 128,
            'conv9', is_BN, trainable)
        if is_train:
          conv9 = tf.nn.dropout(conv9, 0.5)
        fc1 = self.fc_layer(conv9, 128, 'fc1', is_BN, trainable)
        if is_train:
          fc1 = tf.nn.dropout(fc1, 0.5)
        fc2 = self.final_fc_layer(fc1, self.points_num * 2,
                'fc2', trainable)
